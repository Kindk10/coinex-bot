...
### Фото на сервере: почему не отправляются и как исправить

Если скамеры добавлены через импорт из экспорта Telegram, в базе хранится только путь к фото на вашем ПК (`photo_import_path`). На сервере этих файлов нет, поэтому бот не может отправить фото.

**Решение:** один раз на ПК (где есть папка экспорта с фото) запустите:

```bash
npm run backfill-photos
```

Скрипт отправит каждое фото боту в Telegram, получит `file_id` и запишет его в БД. Вам в Telegram придёт серия фото (можно потом удалить). Затем закоммитьте и запушьте `data/bot.db`, на сервере выполните `git pull` и `systemctl restart coinex-bot` — после этого фото будут отправляться и на сервере.

Путь к папке экспорта задаётся в `.env`: `EXPORT_BASE_PATH=путь\к\ChatExport_...` (или по умолчанию используется путь из скрипта).
